...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['mark_claimed'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO student_personal_info_and_detail (student_id, full_name, email, contact, gender, item_type, item_name, category, location, description, status) 
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("sssssssssss", $student_id, $full_name, $email, $contact, $gender, $item_type, $item_name, $category, $location, $description, $status);
      ...
      <div id="cardsContainer">
        ...
        if ($result && $result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                // Always show Unclaimed or Claimed
